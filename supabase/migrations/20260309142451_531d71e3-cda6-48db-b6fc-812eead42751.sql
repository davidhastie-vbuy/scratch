...
BEGIN
  ...
  FOR _customer IN
    SELECT DISTINCT ON (p.id) p.email, p.first_name, j.title AS job_title
    FROM public.jobs j
  ...
  LOOP
    _html := '<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#ffffff;">'
      ...
      || '<p style="font-size:15px;color:#333;">Great news! A new ' || _cat_label || ' provider has joined TradeTrust in your area and may be a great fit for your job "<strong>' || _customer.job_title || '</strong>":</p>'
      ...
      || '</div>'
      || '<p style="font-size:14px;color:#555;">Visit their profile to learn more and invite them to quote on your job.</p>'
    ...
    PERFORM net.http_post(
      ...
      body := jsonb_build_object(
        'to', _customer.email,
        'subject', 'New ' || _cat_label || ' provider available — ' || NEW.business_name,
        'html', _html
      )
    );
  END LOOP;
