...
CREATE OR REPLACE FUNCTION public._supabase_url() RETURNS TEXT LANGUAGE sql IMMUTABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT 'https://gkiohsumxlzqfnnjflgb.supabase.co'::text;
$$;

CREATE OR REPLACE FUNCTION public._supabase_anon_key() RETURNS TEXT LANGUAGE sql IMMUTABLE SECURITY DEFINER SET search_path = public AS $$
