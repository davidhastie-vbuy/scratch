...
CREATE OR REPLACE FUNCTION public.email_provider_on_subsequent_payment()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Only fire for payments AFTER the first (deposit already handled by email_provider_on_first_deposit)
  ...
  _html := '<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#ffffff;">'
    ...
    || '<p style="font-size:15px;color:#333;"><strong>' || _customer_name || '</strong> has paid <strong>£' || TRIM(TO_CHAR(NEW.amount, 'FM999,999,990.00')) || '</strong> for "<strong>' || _milestone_title || '</strong>" on job "<strong>' || _job.title || '</strong>".</p>'
    || '<div style="background:#f0faf0;border-left:4px solid #22863a;padding:16px;margin:16px 0;border-radius:4px;">'
    || '<p style="margin:0 0 8px;font-weight:bold;font-size:15px;color:#22863a;">✓ Payment Received — Ready to Proceed</p>'
    || '<p style="margin:0;font-size:14px;color:#555;">The payment is now held securely in escrow. You can proceed with the next stage of the job.</p>'
    || '</div>'
    || '<p style="font-size:14px;color:#555;">If there are any problems or changes required to the agreed schedule, please contact the customer in advance to discuss.</p>'
  ...
  PERFORM net.http_post(
    ...
    body := jsonb_build_object(
      'to', _provider_email,
      'subject', 'BookATrade: Payment received for "' || _milestone_title || '" — ' || _job.title,
      'html', _html
    )
  );
...
CREATE TRIGGER on_subsequent_payment_email_provider
  AFTER UPDATE ON public.escrow_payments
  FOR EACH ROW
  EXECUTE FUNCTION public.email_provider_on_subsequent_payment();
